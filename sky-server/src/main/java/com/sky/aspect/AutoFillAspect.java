...
//    前置通知
    ...
    public void autoFill(JoinPoint joinPoint) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        //获取当前被拦截的方法的参数，获取实体对象
        AutoFill autoFill=signature.getMethod().getAnnotation(AutoFill.class);//获得方法上的注解对象
        OperationType operationType=autoFill.value();//获得数据类型的操作类型
        ...
        if(operationType==OperationType.INSERT){
            //四个公共字段
            try{                                                        //方法名可以更换,常量替换，避免打字出错
                Method setCreatedTime=entity.getClass().getDeclaredMethod(AutoFillConstant.SET_CREATE_TIME,LocalDateTime.class);
                Method setCreatedUser=entity.getClass().getDeclaredMethod(AutoFillConstant.SET_CREATE_USER,Long.class);
